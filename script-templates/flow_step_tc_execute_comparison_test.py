"""
Flow Step Template: Test Center -> Execute Comparison Test
Script Result Type: text

This workflow template works OOTB after providing values for all required
Variables.

It represents Test Center sub-step -> executing existing comparison test action.

The Script will return Comparison result ID as string.

Either `$comparison_test_id` or `$comparison_test_name` is required.
...
from mstrio.server.test_center.comparison import ComparisonTest
...
BT_ID = $comparison_test_id or None
BT_NAME = $comparison_test_name or None

bt = ComparisonTest(conn, id=BT_ID, name=BT_NAME)
