...
def job_monitors_workflow(conn: "Connection") -> None:
    ...
    # get list of dicts representing job information for
    # all jobs on the environment
    jobs = list_jobs(conn, to_dictionary=True)
    print('All jobs on the environment as dictionaries')
    print(jobs)

    # get a list of `Job` objects filtered by job status,
    ...
    time_hours = 5
    # convert to seconds (for 11.3.2 I-Server version)
    elapsed_t = time_hours * 60**2
    if conn.iserver_version == '11.3.0200':
        # convert to milliseconds (for 11.3.3+ I-Server versions)
        elapsed_t = 1000 * elapsed_t
    # correct form filter (valid operators: 'gt:' and 'lt:')
    elapsed_t = f'gt:{elapsed_t}'
    try:
    ...
    try:
        result = kill_all_jobs(conn, pu_name=PUName.SQL_ENGINE,
                               memory_usage="gt:800")
