...
class FileSystem(Dictable):
    ...
    def __init__(self, create_folder: bool = True, filename_append_time_stamp: bool = True,
                 override_filename: bool = False, append_to_file: bool = False):
...
class ConnectionParameters(Dictable):
    ...
    """
    _DELETE_NONE_VALUES_RECURSION = True
    def __init__(self, retries_count: int = 5, seconds_between_retries: int = 3,
...
class EmailMimeSettings(Dictable):
    ...
    def __init__(self, plain_text_html_body_encoding: Union[EmailEncoding, str],
                 text_attachment_encoding: Union[EmailEncoding, str],
                 binary_attachment_encoding: Union[EmailEncoding, str],
                 message_sensitivity: Union[EmailMessageSensitivity,
                                            str], us_ascii_encoding: bool = False,
                 non_us_ascii_quotes: bool = False, embed_html_attachments: bool = False,
                 embed_all_attachments: bool = False, embed_adobe_flash_content: bool = False,
                 html_table_position_only: bool = False, css_inline_style: bool = False):
        self.plain_text_html_body_encoding = EmailEncoding(
            plain_text_html_body_encoding) if isinstance(plain_text_html_body_encoding,
                                                         str) else plain_text_html_body_encoding
        self.text_attachment_encoding = EmailEncoding(text_attachment_encoding) if isinstance(
            text_attachment_encoding, str) else text_attachment_encoding
        self.binary_attachment_encoding = EmailEncoding(binary_attachment_encoding) if isinstance(
            binary_attachment_encoding, str) else binary_attachment_encoding
        self.message_sensitivity = EmailMessageSensitivity(message_sensitivity) if isinstance(
            message_sensitivity, str) else message_sensitivity
...
class EmailSmartHostSettings(Dictable):
    ...
    def __init__(self, server: Optional[str] = None, port: Optional[int] = None,
                 always_use_smart_host: bool = False, smart_host_username: Optional[str] = None,
...
class EmailDeviceProperties(Dictable):
    ...
    def __init__(self, format: Union[str, EmailFormat],
        ...
        self.mime_settings = EmailMimeSettings.from_dict(mime_settings) if isinstance(
            mime_settings, dict) else mime_settings
        self.smart_host_settings = EmailSmartHostSettings.from_dict(
            smart_host_settings) if isinstance(smart_host_settings, dict) else smart_host_settings
...
class FtpServerSettings(Dictable):
    ...
    def __init__(self, protocol: Union[FtpProtocol,
                                       str], port: int, path: str, host: Optional[str] = None,
                 passive_mode: bool = True, max_connections: int = -1, ascii_mode: bool = True,
...
class FtpDeviceProperties(Dictable):
    ...
    def __init__(self, server_settings: Union[FtpServerSettings, dict],
                 file_system: Union[FileSystem, dict]):
        self.server_settings = FtpServerSettings.from_dict(server_settings) if isinstance(
            server_settings, dict) else server_settings
        self.file_system = FileSystem.from_dict(file_system) if isinstance(file_system,
                                                                           dict) else file_system
...
class IOSDeviceProperties(Dictable):
    ...
    def __init__(self, app_id: str = '', server: Optional[str] = None, port: Optional[int] = None,
...
class AndroidDeviceProperties(Dictable):
    ...
    def __init__(self, package_name: str, server: Optional[str] = None, port: Optional[int] = None,
                 auth_key: Optional[str] = None, collapse_key: Optional[str] = None,
                 delay_with_idle: bool = False, notification_active_hours: Optional[int] = None,
                 use_proxy: bool = False, proxy_server: Optional[str] = None,
...
class FileLocation(Dictable):
    ...
    """
    _DELETE_NONE_VALUES_RECURSION = True
    def __init__(self, file_path: str, append_user_path: bool = False,
                 use_backup_location: bool = False, backup_file_path: Optional[str] = None):
...
class FileProperties(Dictable):
    ...
    def __init__(self, read_only: bool = True, archive: bool = False, index: bool = False,
                 file_encoding: Optional[Union[FileEncoding, str]] = None,
                 unix_access_rights: Optional[str] = None):
        self.read_only = read_only
        self.archive = archive
        self.index = index
        self.file_encoding = FileEncoding(file_encoding) if isinstance(
            file_encoding, str) else file_encoding
...
class UnixWindowsSharity(Dictable):
    ...
    """
    _DELETE_NONE_VALUES_RECURSION = True
    def __init__(self, sharity_enabled: bool = False, server_username: Optional[str] = None,
                 server_password: Optional[str] = None, server_mount_root: Optional[str] = None):
...
class FileDeviceProperties(Dictable):
    ...
    def __init__(self, file_location: Union[FileLocation, dict],
                 file_system: Union[FileSystem, dict],
        ...
        self.file_location = FileLocation.from_dict(file_location) if isinstance(
            file_location, dict) else file_location
        self.file_system = FileSystem.from_dict(file_system) if isinstance(
            file_system, dict) else file_system
        self.connection_parameters = ConnectionParameters.from_dict(
            connection_parameters) if isinstance(
            connection_parameters, dict) else connection_parameters
        self.file_properties = FileProperties.from_dict(file_properties) if isinstance(
            file_properties, dict) else file_properties
...
class PrinterPdfSettings(Dictable):
    ...
    def __init__(self, post_script_level: int = 2, reverse_pages: bool = False,
        ...
        self.post_script_level = (
            post_script_level if 0 <= post_script_level <= 9 else
            exception_handler(
...
class PrinterProperties(Dictable):
    ...
    def __init__(self, pdf_setting: Union[PrinterPdfSettings, dict], quality: Optional[str] = None,
                 scale: int = 100, paper_source: Optional[Union[PrinterPaperSource, str]] = None,
        ...
        self.pdf_setting = PrinterPdfSettings.from_dict(pdf_setting) if isinstance(
            pdf_setting, dict) else pdf_setting
        self.quality = quality
        self.scale = scale
        self.paper_source = PrinterPaperSource(paper_source) if isinstance(
            paper_source, str) else paper_source
        self.paper_size = PrinterPaperSize(paper_size) if isinstance(
            paper_size, str) else paper_size
...
class BackupPrinterProperties(Dictable):
    ...
    def __init__(self, print_on_backup: bool = False,
        ...
        self.backup_location_type = PrinterBackupLocationType(backup_location_type) if isinstance(
            backup_file_location, str) else backup_location_type
...
class PrinterDeviceProperties(Dictable):
    ...
    def __init__(self, printer_location: Union[PrinterLocation,
                                               dict], printer_properties: Union[PrinterProperties,
                                                                                dict],
                 connection_parameters: Union[ConnectionParameters, dict],
                 backup_printer_properties: Union[BackupPrinterProperties,
                                                  dict], temp_file_location: Optional[str] = None):
        self.printer_location = PrinterLocation.from_dict(printer_location) if isinstance(
            printer_location, dict) else printer_location
        self.printer_properties = PrinterProperties.from_dict(printer_properties) if isinstance(
            printer_properties, dict) else printer_properties
        self.connection_parameters = ConnectionParameters.from_dict(
            connection_parameters) if isinstance(
            connection_parameters, dict) else connection_parameters
        self.backup_printer_properties = BackupPrinterProperties.from_dict(
            backup_printer_properties) if isinstance(
            backup_printer_properties, dict) else backup_printer_properties
