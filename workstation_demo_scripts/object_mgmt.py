...
from mstrio.object_management import (CertifiedStatus, Folder, full_search, get_search_results,
                                      list_folders, list_objects, Object, quick_search,
                                      quick_search_from_object, SearchDomain, SearchPattern,
                                      SearchObject, SearchResultsFormat, start_full_search)
...
results = full_search(conn, "project_id", name="document",
                      pattern=SearchPattern.CONTAINS, object_types=ObjectTypes.DOCUMENT_DEFINITION,
                      root="root_folder_id")
print(results)

# return cubes that are used by the given dossier (it can be performed with the
# function `full_search` or method `get_dependencies` from `Entity` class)
dossier = Object(conn, type=ObjectTypes.DOCUMENT_DEFINITION, id="document_id")
print(dossier)
